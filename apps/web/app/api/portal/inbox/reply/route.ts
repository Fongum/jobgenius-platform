...
/**
 * POST /api/portal/inbox/reply
 * Reply to an inbound email using the seeker's connected Gmail.
...
export async function POST(request: Request) {
  ...
  if (!connection) {
    return NextResponse.json(
      { error: "Gmail is not connected. Please reconnect in your profile." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    // Get the original message to extract Message-ID for threading
